...
var (
	...
	tableMap   = map[string]map[string]*obj.TblTable{}
...
// GetTables 获取所有表
func GetTables() map[string]map[string]*obj.TblTable {
	dbLock.RLock()
...
func GetTableAndDB(name string, shard []string) (tables []string,
	...
	namespace, tableName := util.Namespace(name)
...
func SetTable(table *obj.TblTable) {
	...
	if !exists {
		tableMap[table.Name] = map[string]*obj.TblTable{}
	}

	tableMap[table.Name][table.Namespace] = table
}
