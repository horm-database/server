...
func Parse(ctx context.Context, head *proto.RequestHeader, units []*proto.Unit) (resp *proto.QueryResp, err error) {
	...
	executeTree(ctx, head.Appid, tree)
...
// executeTree 执行查询节点
func executeTree(ctx context.Context, appid uint64, node *obj.Tree) {
	for {
		...
		if realNode.IsTransaction() {
			...
			executeTree(ctx, appid, node.TransInfo.Trans)
			finishTrans(node)
			...
			if realNode.Sub == nil { //不包含嵌套子查询
				...
				if node.Finished == consts.QueryFinishedYes && node.IsSuccess() && !node.IsNil {
					...
					for k := 0; k < l; k++ {
						...
						executeTree(ctx, appid, node.SubQuery[k])
					}
				}
			}
		}
